...
public class MainBruteForce {
    ...
    MainBruteForce(final String pdfFilePath, final int startPage, final int endPage) throws IOException {
        ...
        try(final GSFeatures gs=GSFeatures.loadLocal()){
            final List<? extends LinguisticPhenomenon> phenomena= RegexLinguisticPhenomenaReader.read(gs.getModel(), new RegexFeatureQuery().ignoreDeprecated()).getFeatures();
            derivationBuilder=new BruteForceDerivationBuilder(phenomena, derivations);
        }
    }

    MainBruteForce(final String entriesFilePath) throws IOException {
    ...
    public static void main(String[] args) throws IOException {
        final MainBruteForce m=args[0].endsWith(".pdf") ? new MainBruteForce(args[0],121, 1084) : new MainBruteForce(args[0]);
